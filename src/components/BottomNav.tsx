...
const ITEMS = [
  ...
  { path: 'beneficios', label: 'Beneficios', icon: 'local_offer', enabled: true },
        ...
        {ITEMS.map((item) => {
          ...
          return (
            <Link
              key={item.path}
              to={`/${item.path}`}
              className={`relative flex min-h-[44px] min-w-[56px] flex-col items-center justify-center gap-0.5 transition-colors ${
                active
                  ? "font-bold text-ivy-700 after:absolute after:top-0 after:left-1/2 after:h-0.5 after:w-8 after:-translate-x-1/2 after:rounded-full after:bg-cardinal-500 after:content-['']"
                  : 'text-on-surface-variant hover:text-ivy-700'
              }`}
            >
              <span className="material-symbols-outlined text-[22px]">{item.icon}</span>
              <span className="text-[11px] tracking-wide">{item.label}</span>
